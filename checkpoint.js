...
function sumaTodosImpares(array) {
  ...
  // Tu código aca:
    ...
    return array.reduce((i, n) => (n % 2 !== 0 ? i + n : i  ), 0);
...
function stringMasLarga(str) {
  ...
  // Tu código aca:
   let string0 = str.split(" ");
   let string1 = "";
   for (let stringlarga of string0) {
     if(stringlarga.length > string1.length) {
       string1 = stringlarga;
     }
   }
   return string1;
}
console.log(stringMasLarga("hola mi nombre es juan sebastian"));
...
function estaOffline(usuarios, nombre) {
  ...
  (personas[usuario.nombre] = usuario.online))
...
function crearClaseEmprendedor() {
  class Emprendedor {
      ...
      getBooks() {
          ...
          // Tu código aca:
          let array= []  
          this.libros.map((obj) => array.push(obj['nombre']))
          return array; 
      }
